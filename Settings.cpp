...
Settings::Settings( std::string dir, std::string service, int instance, bool ignore_cache )
{
	this->dir = dir;
...
bool Settings::loadVars( bool ignore_cache )
{
	...
	char cmd[ 256 ];
	cmd[255] = 0;
	snprintf(cmd, 255, "%s %s %s", script_loadvars, dir.c_str(), service.c_str() );
	FILE *fp = popen( cmd, "r" );
	if( !fp )
		return false;
	char buf[4096];
...
Settings SettingsDir::byService(std::string service, int instance, bool ignore_cache)
{
