...
std::string fieldByName(const std::map<std::string, std::string> &st,
		...
		const std::vector<std::map<std::string, std::string> > &gpu_info)
{
	...
	for(unsigned i=0; i < gpu_info.size(); i++){
		...
		table->elementAt(i+1, 0)->addWidget(new WText(fieldByName(f, "pciid", "")));
		table->elementAt(i+1, 1)->addWidget(new WText(fieldByName(f, "name", "")));
		...
		w_coreclk->save_button->clicked().connect( boost::bind(
				&NvidiaPage::saveCoreClock, this, uuid, w_coreclk->line_edit ));
		w_memclk->save_button->clicked().connect( boost::bind(
				&NvidiaPage::saveMemClock, this, uuid, w_memclk->line_edit ) );
		w_fan->save_button->clicked().connect( boost::bind(
				&NvidiaPage::saveFan, this, uuid, w_fan->line_edit ) );
		w_plim->save_button->clicked().connect( boost::bind(
				&NvidiaPage::savePlim, this, uuid, w_plim->line_edit ) );

		w_coreclk->apply_button->clicked().connect( boost::bind(
				&NvidiaPage::applyCoreClock, this, uuid, w_coreclk->line_edit ) );
		w_memclk->apply_button->clicked().connect( boost::bind(
				&NvidiaPage::applyMemClock, this, uuid, w_memclk->line_edit ) );
		w_fan->apply_button->clicked().connect( boost::bind(
				&NvidiaPage::applyFan, this, uuid, w_fan->line_edit ) );
		w_plim->apply_button->clicked().connect( boost::bind(
				&NvidiaPage::applyPlim, this, uuid, w_plim->line_edit ) );
	}
}
