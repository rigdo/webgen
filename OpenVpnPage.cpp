...
//===================== OpenVpnPage ======================================
//==============================================================================
OpenVpnPage::OpenVpnPage( SettingsDir *sd, int idx ) : BasePage()
{
	this->sd = sd;
	this->idx = idx;
	...
	ftable = new UploadedFilesTable( s.additionalSettingsDirName("usrconfig").toUTF8() );
...
//	statickey_textarea->setValidator( validator );
	...
	cacrt_fileupload = new UploadedFileWidget( 
			s.additionalSettingsDirName("guiclientconfig").toUTF8(), "ca.crt", "ca(ca.crt)" );
	takey_fileupload = new UploadedFileWidget( 
			s.additionalSettingsDirName("guiclientconfig").toUTF8(), "ta.key", "tls-auth(ta.key)" );
	...
	clientcrt_fileupload = new UploadedFileWidget( 
			s.additionalSettingsDirName("guiclientconfig").toUTF8(), "client.crt", "cert(client.crt)" );
	clientkey_fileupload = new UploadedFileWidget( 
			s.additionalSettingsDirName("guiclientconfig").toUTF8(), "client.key","key(client.key)" );
	...
	servcacrt_fileupload = new UploadedFileWidget( 
			s.additionalSettingsDirName("guiservconfig").toUTF8(), "ca.crt", "ca(ca.crt)" );
	servcrt_fileupload = new UploadedFileWidget( 
			s.additionalSettingsDirName("guiservconfig").toUTF8(), "server.crt", "cert(server.crt)" );
	servkey_fileupload = new UploadedFileWidget( 
			s.additionalSettingsDirName("guiservconfig").toUTF8(), "server.key","key(server.key)" );
	servtakey_fileupload = new UploadedFileWidget( 
			s.additionalSettingsDirName("guiservconfig").toUTF8(), "ta.key", "tls-auth(ta.key)" );
	dhkey_fileupload = new UploadedFileWidget( 
			s.additionalSettingsDirName("guiservconfig").toUTF8(), "dh1024.pem", "dh(dh1024.pem)" );
...
//			&OpenVpnPage::enableChanged );
	...
	workmode_combobox->activated().connect( this, 
			&OpenVpnPage::workModeChanged );
	authby_combobox->activated().connect( this, 
			&OpenVpnPage::authByChanged );
...
void OpenVpnPage::workModeChanged( int newval )
{
	switch( newval ){
		...
		case 1: //Server
			...
			help_text->setText( WString(tr("OpenVpn_server_help")).arg("tls-auth(ta.key)").arg(tr("Additionl options")) );
			break;
		case 2: //P2P Client
			...
			connectport_container->show();
			listenport_container->hide();
			help_text->setText( WString(tr("OpenVpn_p2p_help")).arg(tr("Encryption key")) );
			break;
		case 3: //P2P Server
			p2psettings_groupbox->show();
			clientsettings_groupbox->hide();
...
void OpenVpnPage::authByChanged( int newval )
{
	if( newval == 0  ) { // by key
		...
		help_text->setText( WString(tr("OpenVpn_client_bykey_help")).arg("tls-auth(ta.key)").arg(tr("Additionl options")) );
	}
	else{ //by password
		authbykey_container->hide();
		authbypass_container->show();
		help_text->setText( WString(tr("OpenVpn_client_bypass_help")).arg("tls-auth(ta.key)").arg(tr("Additionl options")) );
	}
}
...
void OpenVpnPage::loadParams()
{
	...
	configmode_combobox->setCurrentIndex( 
			s.valueInt("OPENVPN_CONFIGMODE", 0) );
	
	int proto_idx = proto_combobox->findText( s.value("OPENVPN_PROTOCOL", "udp") );
	if( proto_idx < 0 )
		proto_idx = 0;
	proto_combobox->setCurrentIndex( proto_idx );

	int devtype_idx = devtype_combobox->findText( s.value("OPENVPN_DEVTYPE", "tun") );
	if( devtype_idx < 0 )
		devtype_idx = 0;
	devtype_combobox->setCurrentIndex( devtype_idx );
	
	int cipher_idx = cipher_combobox->findText( s.value("OPENVPN_CIPHER", "BF-CBC") );
	if( cipher_idx < 0 )
		cipher_idx = 0;
	cipher_combobox->setCurrentIndex( cipher_idx );
	
	workmode_combobox->setCurrentIndex( 
			s.valueInt("OPENVPN_WORKMODE", 0) );
	connectaddr_lineedit->setText( 
			s.value("OPENVPN_CONNECTADDR", "127.0.0.1"));
	...
	authby_combobox->setCurrentIndex( 
			s.valueInt("OPENVPN_AUTHBY", 0) );
...
void OpenVpnPage::saveParams()
{
	...
	s.save( "OPENVPN_CLIENTADDITIONALOPTS", additionaloptsclient_textarea->text());
